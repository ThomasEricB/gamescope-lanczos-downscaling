	...
	struct FrameInfo_t *frameInfo )
{
	...
	if ( subtex && subtex->buf )
	{
		...
		FrameInfo_t::Layer_t *sublayer = pSubTex ? frameInfo->layers.push() : nullptr;
		if ( sublayer )
		{
			*sublayer = FrameInfo_t::Layer_t{};
		...
		if ( w )
		{
			...
			else
			{
				...
				else
				{
					...
					bool needsDownscale = frameInfo.layers.get( 0 ).scale.x > 1.001f && frameInfo.layers.get( 0 ).scale.y > 1.001f;
...
handle_property_notify(xwayland_ctx_t *ctx, XPropertyEvent *ev)
{
	...
	if ( ev->atom == ctx->atoms.gamescopeNewScalingFilter )
	{
		...
		if ( uScalingFilter > uint32_t( GamescopeUpscaleFilter::LANCZOS ) )
		{
			xwm_log.errorf( "Unknown scaling filter %u, keeping %u", uScalingFilter, uint32_t( g_wantedUpscaleFilter ) );
		}
		else
		{
			GamescopeUpscaleFilter nScalingFilter = GamescopeUpscaleFilter( uScalingFilter );
	...
	if ( ev->atom == ctx->atoms.gamescopeNewScalingScaler )
	{
		...
		{
			GamescopeUpscaleScaler nScalingScaler = GamescopeUpscaleScaler( uScalingScaler );
...
void update_wayland_res(CommitDoneList_t *doneCommits, steamcompmgr_win_t *w, ResListEntry_t& reslistentry)
{
	...
	if ( bPreemptiveUpscale )
	{
		...
		if ( pTempImage )
		{
			...
			if ( cv_upscale_preemptive_debug_force_sync && seqNo )
			{
